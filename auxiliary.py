...
class AuxiliaryTaskManager:
    ...
    def __init__(self, actor, obs_dim, sr_weight=1.0, rp_weight=1.0,
        ...
        # Reduced maxlen to mitigate high memory usage
    ...
    def set_learning_mode(self, mode):
        ...
        if prev_mode != mode:
            if mode == "batch":
                ...
                self.feature_history = deque(maxlen=self.batch_history_maxlen)
                ...
                temp_features = list(self.feature_history)[-self.stream_history_maxlen:] if self.feature_history else []
                ...
                self.feature_history = deque(maxlen=self.stream_history_maxlen)
