...
def kill_child_processes(parent_pid, timeout=2, debug=False):



    """Attempts graceful termination (SIGTERM) first, then forceful (SIGKILL)."""
    ...
    try:
        ...
        if not children:
            ...
            if debug: print(f"[DEBUG kill_child_processes] No child processes found for PID {parent_pid}.")
        ...
        if debug: print(f"[DEBUG kill_child_processes] Attempting graceful termination (SIGTERM) for {len(children)} children...")
        ...
        for child in children:
            ...
            try:
                ...
                if child.is_running():
                    ...
                    if debug: print(f"[DEBUG kill_child_processes] Sent SIGTERM to child PID {child.pid} ({child.name()})")


            except psutil.NoSuchProcess:


                if debug: print(f"[DEBUG kill_child_processes] Child PID {child.pid} already gone before SIGTERM.")


            except Exception as e:


                print(f"Error sending SIGTERM to child process {child.pid}: {e}")
        ...
        if debug: print(f"[DEBUG kill_child_processes] {len(gone)} children terminated gracefully. {len(alive)} still alive.")
        ...
        if alive:
            ...
            if debug: print(f"[DEBUG kill_child_processes] Forcefully terminating remaining {len(alive)} children (SIGKILL)...")
            ...
            for child in alive:
                ...
                try:
                    ...
                    if child.is_running():
                        ...
                        if debug: print(f"[DEBUG kill_child_processes] Sent SIGKILL to child PID {child.pid} ({child.name()})")


                except psutil.NoSuchProcess:


                     if debug: print(f"[DEBUG kill_child_processes] Child PID {child.pid} gone before SIGKILL.")


                except Exception as e:


                    print(f"Error sending SIGKILL to child process {child.pid}: {e}")
            ...
            gone, alive_after_kill = psutil.wait_procs(alive, timeout=1.0)


            if alive_after_kill:


                 if debug: print(f"[DEBUG kill_child_processes] WARNING: {len(alive_after_kill)} children still alive after SIGKILL!")


            elif debug:


                 print(f"[DEBUG kill_child_processes] All remaining children terminated.")
        ...
        if debug: print(f"[DEBUG kill_child_processes] Parent process {parent_pid} not found.")


    except Exception as e:


        print(f"Error finding/killing child processes: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # --- Final Child Process Cleanup (SIGTERM then SIGKILL) ---
        main_pid = os.getpid()
        if args.debug: print("[DEBUG] Performing final child process cleanup (TERM then KILL)...")
        kill_child_processes(main_pid, timeout=2, debug=args.debug) # Use 2 second timeout for SIGTERM
        if args.debug: print("[DEBUG] Final child process cleanup finished.")

        # Optional: Check active children again after cleanup
        if args.debug:
            time.sleep(1) # Give OS time to update process list
            try:
                current_process = psutil.Process(main_pid)
                final_children = current_process.children(recursive=True)
                print(f"[DEBUG] Final check: {len(final_children)} child processes remaining.")
                for child in final_children:
                    print(f"  - Remaining Child PID: {child.pid} ({child.name()})")
            except psutil.NoSuchProcess:
                    print("[DEBUG] Main process already exited.")
            except Exception as e:
                    print(f"[DEBUG] Error during final child check: {e}")
